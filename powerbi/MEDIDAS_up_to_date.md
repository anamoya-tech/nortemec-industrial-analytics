...
**Versión actualizada:** medidas base del modelo + medidas específicas del Informe 1 — Rentabilidad y margen.
...
Este documento unifica las medidas base creadas inicialmente con las medidas añadidas durante la construcción de la primera página del informe de rentabilidad.
...
## 9. Medidas añadidas para el Informe 1 — Rentabilidad y margen

Durante la construcción de la **Página 1 — Resumen Ejecutivo de Rentabilidad**, se añadieron medidas específicas para mejorar la lectura ejecutiva del dashboard.

Estas medidas complementan las medidas base de ventas y rentabilidad y permiten identificar pedidos problemáticos, aplicar formato semáforo al KPI de margen y reforzar la interpretación visual de la página.

---

### Pedidos problemáticos

Esta medida cuenta los pedidos que presentan al menos uno de estos problemas:
...
```text
Tarjeta KPI — Pedidos problemáticos
```

Esta medida ayuda a responder:

```text
¿Cuántos pedidos presentan riesgo comercial u operativo?
```
...
Esta medida ayuda a dimensionar el peso de los pedidos problemáticos sobre el total de pedidos.

---

### Color fondo % margen

Esta medida se creó para aplicar formato condicional al fondo de la tarjeta de **% Margen**.
...
| Rango de margen | Color | Código | Interpretación |
|---|---|---:|---|
| `< 0 %` | Rojo | `#C62828` | Pérdida |
| `0 % - 10 %` | Naranja | `#EF6C00` | Margen bajo |
| `10 % - 20 %` | Amarillo | `#FFD54F` | Margen medio |
| `>= 20 %` | Verde | `#2E7D32` | Margen sano |
| Sin dato | Gris | `#7A8691` | Sin información suficiente |

Uso recomendado en Power BI:

```text
Tarjeta % Margen
Formato visual → Efectos → Fondo → fx
Formato por: Valor de campo
Basado en campo: Color fondo % margen
```

Motivo de la decisión:

```text
El semáforo se integró directamente en el KPI de margen para ahorrar espacio y reforzar la lectura ejecutiva.
```
...
Esta medida devuelve una etiqueta textual del estado del margen.
...
SWITCH (
    TRUE(),
    [% Margen] < 0, "Pérdida",
    [% Margen] < 0.10, "Margen bajo",
    [% Margen] < 0.20, "Margen medio",
    [% Margen] >= 0.20, "Margen sano",
    "Sin dato"
)
```
...
Esta medida se dejó como alternativa textual para usar en tarjetas, tablas o tooltips.
...
Aunque finalmente se priorizó el uso de `Color fondo % margen` en la tarjeta principal, esta medida puede ser útil en otras páginas del informe.

---

## 10. Medidas pendientes para próximas páginas

No se crearon todavía todas las medidas ampliadas porque se decidió construir el dashboard de forma progresiva: crear medidas solo cuando sean necesarias para cada página.

Quedan pendientes para próximas páginas:

```text
Objetivos mensuales
Costes reales
Stock y riesgo de rotura
...
Ejemplos de medidas futuras:

```text
Objetivo facturación
Desviación facturación
% cumplimiento objetivo facturación
Coste real total
Desviación coste real
Órdenes con sobrecoste
Materiales en riesgo de stock
Valor necesario a reponer
Acciones correctivas abiertas
% acciones efectivas
Preventivos programados
% cumplimiento preventivo
Clientes alto potencial
Clientes riesgo alto
Gap potencial comercial
```

Estas medidas se incorporarán a medida que se construyan las siguientes páginas del informe.
...
```text
Fase 5 — Medidas DAX base: COMPLETADA
Medidas específicas Página 1 — Rentabilidad y margen: COMPLETADAS
Medidas ampliadas por nuevas tablas: PENDIENTES SEGÚN NECESIDAD DEL INFORME
```
